...
public abstract class Simulator {
  ...
  public double printStats(boolean doPrint) {
    ...
    for (Job j : jobs) {
      ...
      if (doPrint) {
        System.out.println(j.jobName + "\tstart time: " + j.simulatedStartTime + "\tfinish time: " + j.simulatedFinishTime
            + "\tmapper number: " + j.numMappers + "\treducer number: " + j.numReducers + "\tshuffle bytes: " + j.totalShuffleBytes + "\tmax shuffle bytes: "
            + j.maxShuffleBytes + "\tjob duration: " + jDur + "\tdeadline duration: " + Math.round(j.deadlineDuration) + "\tshuffle individual sums: "
